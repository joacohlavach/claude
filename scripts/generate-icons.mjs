...
// Geometric "G": ring arc with a right-facing gap + inward bar, centered on (32,32).
const RING = 'M 41.96 23.64 A 13 13 0 1 0 41.96 40.36'
const BAR = 'M 43 32 L 32 32'
...
function badgeSvg({ size = 64, radius = 0, scale = 1 } = {}) {
  return `<svg width="${size}" height="${size}" viewBox="0 0 64 64" xmlns="http://www.w3.org/2000/svg">
    <defs>
      <linearGradient id="g" gradientUnits="userSpaceOnUse" x1="19" y1="19" x2="45" y2="45">
        <stop offset="0%" stop-color="#e2895a" />
        <stop offset="55%" stop-color="#d97a44" />
        <stop offset="100%" stop-color="#a85128" />
      </linearGradient>
    </defs>
    <rect x="0" y="0" width="64" height="64" rx="${radius}" fill="#101010" />
    <g transform="translate(32 32) scale(${scale}) translate(-32 -32)">
      <path d="${RING}" fill="none" stroke="url(#g)" stroke-width="6.4" stroke-linecap="round" />
      <path d="${BAR}" fill="none" stroke="url(#g)" stroke-width="6.4" stroke-linecap="round" />
    </g>
  </svg>`
}
